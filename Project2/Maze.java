...
public class Maze {
  ...
  public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
   ...
   sc.push(startState);
    //markVisited(startState,visited);
	//visitedNodes.addBehind(startState);
   ...
   while(!sc.isEmpty()) {
    	...
    	if(isExit(currentState.x,currentState.y)) {
    		...
    		if(CanImove(currentState.x,currentState.y-1)) {
    			
    			nextState= new Coordinate(currentState.x,currentState.y-1);
    			sc.push(nextState);
    			
    		}
    		
    		
    		if(CanImove(currentState.x+1,currentState.y)) {
    			
    			nextState= new Coordinate(currentState.x+1,currentState.y);
    			sc.push(nextState);
    		}
  ...
  private boolean CanImove (int row,int col) {
	  Coordinate myCoord = new Coordinate(row,col);
	  return(isInMaze(myCoord) && currentMaze[row][col]!= I && currentMaze[row][col] != visited); 
  ...
  private boolean isInMaze(Coordinate c) {
	  return(c.x>=0 && c.y>=0 && c.x<currentMaze.length && c.y<currentMaze[0].length); 
