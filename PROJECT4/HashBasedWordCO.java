...
public class HashBasedWordCO {
    ...
    for (String sentence : sentences) {
      for (String word : sentence.split("\\s+")) {
    	// TODO: Fill here
          ...
          // the minWordLen
       if(word.length() >= minWordLen){uWords.put(word);}
       if(IsItIgnored(word)) {uWords.remove(word);}
        }
      }
    return uWords;
  }
  ...
  public boolean IsItIgnored(String word) {
	  
	  for(int k=0; k<ignoreList.length; k++) { //If any word happens to be in the ignoreList, return true;. 
	  		if(ignoreList[k].equals(word)) 
	  			return true;
  ...
  public void fillCoMat(String text, String[] keyWords) {
    ...
    for(String theSentence : sentences) {
        String[] words = theSentence.split("\\s+");

        for(int i = 0; i<words.length ;i++) {
          String word = words[i];

          if (coMat.get(word) != null) {
            int starting = (i-windowSize);
            ...
            for (int k = starting; k <= windowSize + i; k++) {
              if (k!=i && k< words.length) {
                String Word = words[k];
                coMat.get(word).increment(Word); 
                }
              }
          }
        }
